...
for(samp in 1:B) {
  ...
  if(near(mean_dem_pct_PA, sample_delegation_pct_dem_mean, 0.1)){
    ...
    samples_diff[samp] <- (sample_delegation_pct_dem_median - sample_delegation_pct_dem_mean)
  }
}

samples <- data.frame(samples_mean,samples_median,samples_diff)
samples <- filter_all(samples, any_vars(. != 0))
quantile(samples$samples_median, c(0.10,1))
quantile(samples$samples_diff, c(0.10,1))
