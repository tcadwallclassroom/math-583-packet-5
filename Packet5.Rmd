...
library(gghighlight)
library(formatR)
knitr::opts_chunk$set(echo = TRUE, cache = TRUE, eval = TRUE, cache.lazy = FALSE)
...
The main idea I want to get across is that there are at least 3 ways to run a hypothesis test: 

1. Simulation-based tests
2. Critical value (based on a known probability distribution
3. P-value (based on a known probability distribution)

In R, a simulation-based test has a lot of advantages. A critical value can be calculated using `q` functions (like `qnorm` or `qbinom`), while a p-value can be found with `p` functions like `pnorm` or `pbinom`. 
